...
if fatload mmc 0 ${initrd_loadaddr} uInitrd; then if fatload mmc 0:1 ${kernel_loadaddr} uImage; then if fatload mmc 0 ${dtb_loadaddr} dtb.img; then run boot_start;fi;fi;fi;
if fatload usb 0 ${initrd_loadaddr} uInitrd; then if fatload usb 0:1 ${kernel_loadaddr} uImage; then if fatload usb 0 ${dtb_loadaddr} dtb.img; then run boot_start;fi;fi;fi;
